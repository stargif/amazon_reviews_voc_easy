...
function getAnimationType(progressText: string): AnimationType {
  // 步骤1: 数据准备阶段 (验证/解析 CSV)
  if (
    progressText.includes("验证") ||
    progressText.includes("解析")
  ) {
    return "data-stream";
  }
  // 步骤2: AI分析阶段 (生成标签体系)
  if (progressText.includes("标签体系") || progressText.includes("AI分析")) {
    return "fluid";
  }
  // 步骤3: 逐条处理阶段 (分析评论)
  if (progressText.includes("分析评论")) {
    return "crystal";
  }
  // 步骤4: 结果生成阶段 (词频/画像/CSV)
  if (
    progressText.includes("词频") ||
    progressText.includes("用户画像") ||
    progressText.includes("persona") ||
    progressText.includes("生成CSV") ||
    progressText.includes("生成结果") ||
    progressText.includes("完成") ||
    progressText.includes("成功")
  ) {
    return "neural";
  }
  return "data-stream";
...
function getProgressPercent(progressText: string): number {
  // 步骤1: 验证/解析 CSV (5-15%)
  if (progressText.includes("验证")) return 5;
  if (progressText.includes("解析")) return 10;
  
  // 步骤2: 生成标签体系 (20-25%)
  if (progressText.includes("标签体系") || progressText.includes("AI分析")) return 25;
  ...
  if (progressText.includes("分析评论")) {
    ...
    if (match) {
      ...
      return 30 + (current / total) * 50;
    }
    return 30;
  }
  
  // 步骤4: 计算词频 (85%)
  if (progressText.includes("词频")) return 85;
  
  // 步骤5: 生成用户画像 (90%)
  if (progressText.includes("用户画像") || progressText.includes("persona")) return 90;
  
  // 步骤6: 生成CSV (95%)
  if (progressText.includes("生成CSV") || progressText.includes("生成结果")) return 95;
  
  // 完成 (100%)
  if (progressText.includes("完成") || progressText.includes("成功")) return 100;
...
function getLogDetail(progressText: string): string {
  // 步骤1: 验证/解析 CSV
  if (progressText.includes("验证")) return "正在验证CSV文件格式...";
  if (progressText.includes("解析")) return "正在解析CSV文件内容...";
  
  // 步骤2: 生成标签体系
  if (progressText.includes("标签体系") || progressText.includes("AI分析")) {
    return "AI正在分析评论特征，生成标签体系...";
  }
  ...
  if (progressText.includes("分析评论")) {
    const match = progressText.match(/(\d+)\/(\d+)/);
    if (match) {
      return `正在分析第 ${match[1]}/${match[2]} 条评论...`;
    }
    return "正在逐条分析评论内容...";
  }
  
  // 步骤4: 计算词频
  if (progressText.includes("词频")) return "正在统计标签词频...";
  
  // 步骤5: 生成用户画像
  if (progressText.includes("用户画像") || progressText.includes("persona")) {
    return "正在生成用户画像分析...";
  }
  
  // 步骤6: 生成CSV
  if (progressText.includes("生成CSV") || progressText.includes("生成结果")) {
    return "正在生成分析结果文件...";
  }
  
  // 完成
  if (progressText.includes("完成") || progressText.includes("成功")) {
    return "分析完成！";
  }
